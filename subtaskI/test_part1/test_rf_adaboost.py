...
for seed in [0, 1, 2, 3, 4]:
    ...
    clf = MultiOutputClassifier(AdaBoostClassifier(estimator=base_tree, n_estimators=100, random_state=seed))
    ...
    df_out = pd.DataFrame({'predicted_labels': [str(labs) for labs in y_pred_str]})
    ...
    df_test_out = pd.DataFrame({'predicted_labels': [str(labs) for labs in y_test_pred_str]})
